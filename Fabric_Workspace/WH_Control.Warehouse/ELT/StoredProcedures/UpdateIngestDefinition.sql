							...
							CASE
								...
								WHEN @LastDeltaDate IS NOT NULL AND @ReloadFlag <> 1 AND @IngestStatus IN ('Success','ReRunSuccess') AND @LastDeltaDate = [LastDeltaDate] 
									...
									THEN 
										CASE 
											WHEN 
												DateAdd(minute,[MaxIntervalMinutes],@LastDeltaDate) > CONVERT(datetime2,CONVERT(datetimeoffset, getdate()) AT TIME ZONE 'AUS Eastern Standard Time')
													THEN CONVERT(VARCHAR(30),CONVERT(datetime2,CONVERT(datetimeoffset, getdate()) AT TIME ZONE 'AUS Eastern Standard Time'),120)
											ELSE
