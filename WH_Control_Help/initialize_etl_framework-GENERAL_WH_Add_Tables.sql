...
FROM (
    SELECT 'dbo' AS Schema_Name, 'F1ASR_REG_ASSET_ATTR' AS Table_Name, 'ASSNBRI|ATTR_CODE|ATTR_UNIQUE_ID|REG_NAME' AS primary_key UNION 
    SELECT 'dbo' AS Schema_Name, 'F1ASR_REG_ASSET_UF' AS Table_Name, 'ASSNBRI|REG_NAME' AS primary_key  
) A;
...
SELECT
    (select max(IngestID) from [ELT].[IngestDefinition]) + ROW_NUMBER() OVER (ORDER BY Table_Name) as IngestID,
    t.SourceSystemName,
...
SELECT 
(select max(L1TransformID) from [ELT].[L1TransformDefinition]) + ROW_NUMBER() OVER (ORDER BY source.IngestID) as L1TransformID,
source.IngestID, 
NULL AS [ComputePath],
...
'initial_load' AS [ModifiedBy],
GETDATE() AS [ModifiedTimestamp]
FROM [ELT].[IngestDefinition] source
left JOIN [ELT].[L1TransformDefinition] target 
ON source.IngestID = target.IngestID
INNER JOIN ELT.Z_Fabric_Metadata t ON 1=1
WHERE target.IngestID IS NULL;
