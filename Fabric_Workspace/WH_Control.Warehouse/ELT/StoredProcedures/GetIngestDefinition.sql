	...
	with CTE as (
		...
		--Stats Query
			, StatSQL = 
				...
				CASE 
					...
					--Common No Delta
						WHEN [EntityName] IS NOT NULL AND [WatermarkColName] IS NULL
							THEN 'SELECT ''1900-01-01 00:00:00'' AS DataFromTimestamp, ''' + CONVERT(VARCHAR(30),CONVERT(datetime,CONVERT(datetimeoffset, getdate()) at time zone 'AUS Eastern Standard Time'),120) + ''' AS DataToTimestamp,  COUNT(*) AS SourceCount FROM ' + [EntityName]
					--Running Number
						WHEN [EntityName] IS NOT NULL AND [WatermarkColName] IS NOT NULL AND [LastDeltaNumber] IS NOT NULL
		...
		UNION
			...
			, StatSQL = 
				...
				--Common No Delta
					WHEN [EntityName] IS NOT NULL AND [WatermarkColName] IS NULL
						THEN 'SELECT SELECT ''1900-01-01 00:00:00'' AS DataFromTimestamp, ''' + CONVERT(VARCHAR(30),CONVERT(datetime,CONVERT(datetimeoffset, getdate()) at time zone 'AUS Eastern Standard Time'),120) + ''' AS DataToTimestamp, COUNT(*) AS SourceCount FROM ' + [EntityName]
				--Running Number
					WHEN [EntityName] IS NOT NULL AND [WatermarkColName] IS NOT NULL AND [LastDeltaNumber] IS NOT NULL
							THEN 'SELECT MIN('+[WatermarkColName]+') AS DataFromTimestamp,' + ' MAX('+[WatermarkColName]+') AS DataToTimestamp,'+ 'COUNT(*) AS SourceCount FROM ' + [EntityName]
											+ [WatermarkColName] + ' > ' + ''''+CONVERT(VARCHAR,II.[DataFromNumber]) +'''' + ' AND ' + [WatermarkColName] + ' <= ' + ''''+CONVERT(VARCHAR,II.[DataToNumber])+''''
			...
			FROM
				...
				ON II.[IngestID]= ID.[IngestID]
					AND II.[ReloadFlag]=1
					AND (II.[IngestStatus] is NULL OR II.[IngestStatus] != 'Running')
			--Fetch new instances and ignore instances that are currently running
			WHERE 
